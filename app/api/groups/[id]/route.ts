...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.videoIds !== undefined) { const videoIds = await validateVideoIds(body.videoIds); if (!videoIds) return NextResponse.json({ error: "Pilihan video tidak sah." }, { status: 400, headers: noStore }); const module = await sql()`SELECT id FROM group_modules WHERE group_id = ${id} ORDER BY sort_order ASC LIMIT 1`; await sql()`DELETE FROM video_groups WHERE group_id = ${id}`; for (const [index, videoId] of videoIds.entries()) await sql()`INSERT INTO video_groups (video_id, group_id, module_id, sort_order) VALUES (${videoId}, ${id}, ${module[0].id}, ${index})`; }
